...
| **X-Engine** | PolarDB 8.0.x | 自研 | Tiered (2-4路) | **FPGA 加速 Compaction** | 低 | 低 |
...
LevelDB 是 2011 年 Jeff Dean 和 Sanjay Ghemawat 编写的精简 LSM 实现，~15000 行 C++，定义了业界标准范式。
...
| 10.2 | **HyperClockCache** 成默认 | 无锁并发缓存，替代 LRU |
...
**事务处理流水线**: 阶段并行化 → 吞吐相比 RocksDB 10x+。支撑 2018 双 11 49.1 万笔/秒。
...
1996: LSM-Tree 论文 (O'Neil, UMass Boston)
  ...
  │    └─ 2008: Apache HBase (Bigtable 开源复刻, 暴露所有 LSM 经典问题)
  │
  ...
  │
  ├─ 2009: Cassandra (分布式 LSM)
  │    ├─ 2015: ScyllaDB ── C++ 重写 + Shard-per-Core + ICS
  │    └─ 2024: Cassandra 5.0 UCS ── 统一策略
  │
  ├─ 2014: OceanBase V1 ── 自研 LSM + Macro Block + 轮转合并
  │    └─ 2024: V4.3+ 混合行列存
  │
  ├─ 2017: Doris ── 列式 LSM → 2023: MoW
  ├─ 2020: StarRocks ── Primary Key + DelVector
  │
  └─ 数据湖:
      ├─ 2017: Hudi ── MOR (类 LSM) → 2025: LSM Timeline
      └─ 2023: Paimon ── 唯一湖格式内置 LSM + 三模式
```
